...
PAKISTAN_PROVINCE_CODES = [
    "ICT", "ISB", "RWP", "LHR", "KHI", "PEW", "SWL", "FSD", "MULTAN", 
    "PESH", "QUETTA", "HYD", "SUKKUR", "DGK", "BWP", "SAHIWAL", "GUJRANWALA",
    "SARGODHA", "MARDAN", "ABBOTABAD", "MINGORA", "GILGIT", "SKARDU", "TURBAT"
]
...
# Load Haar cascade for license plate detection as fallback
plate_cascade = None
try:
    ...
    def _detect_and_recognize(self, frame):
        ...
        for p in persons:
            ...
            if not is_authorized:
                ...
                if logged:
                    # Save authorized face image for display
...
class PlateWorker(WorkerBase):
    ...
    def _validate_plate(self, text: str, box: List[int]) -> bool:
        ...
        for province in PAKISTAN_PROVINCE_CODES:
    ...
    def _detect_and_read(self, frame):
        ...
        # If YOLO model failed or not available, try Haar cascade
        ...
        if not seen_any and plate_model is None and plate_cascade is None:
...
<head>
  ...
  <title>AI Security Dashboard – Face & Pakistan Plate Recognition</title>
  ...
  <style>
    ...
    * {
    ...
    .pakistan-flag {
      width: 24px;
      height: 16px;
      background-image: url('data:image/svg+xml;base64,');
...
<body>
  <div class=\"container\">
    <header>
      ...
      <p>Advanced Face & Pakistan License Plate Recognition System</p>
    </header>
    ...
    <div class=\"dashboard-grid\">
      ...
      <div class=\"card\" style=\"grid-column: span 2;\">
        <div class=\"card-header\">
          <h3><i class=\"fas fa-user-friends\"></i> Known Faces</h3>
          <div class=\"flex\">
            <button class=\"btn btn-sm\" onclick=\"openAddFace()\"><i class=\"fas fa-plus\"></i> Add Face</button>
            <button class=\"btn btn-sm\" onclick=\"reloadIndex()\"><i class=\"fas fa-rotate\"></i> Rebuild</button>
          </div>
        </div>
        <div class=\"card-body\">
      ...
      <div class=\"card\" style=\"grid-column: span 2;\">
        <div class=\"card-header\">
          <h3><i class=\"fas fa-clipboard-check\"></i> Authorized Pakistan Plates</h3>
          <div class=\"flex\">
            <input id=\"plate-input\" class=\"input\" placeholder=\"Enter plate e.g. ABC-123\"> 
            <input id=\"plate-label\" class=\"input\" placeholder=\"Label\"> 
            <button class=\"btn btn-sm\" onclick=\"addPlateUI()\"><i class=\"fas fa-plus\"></i> Add</button>
            <button class=\"btn btn-sm\" onclick=\"loadPlates()\"><i class=\"fas fa-sync-alt\"></i></button>
          </div>
        </div>
        <div class=\"card-body\">
          <div class=\"table-container\">
  ...
  <div id=\"add-face-modal\" class=\"modal\"> 
    <div class=\"modal-content\">
      <div class=\"modal-header\">
        <h3>Add New Face</h3>
        <button class=\"btn btn-sm\" onclick=\"closeAddFace()\"><i class=\"fas fa-times\"></i></button>
      </div>
  ...
  <script>
    ...
    function updateBarrierStatus() {
      fetch('/barrier_status')
        ...
        .then(data => {
          ...
          // Show authorized person if available
          ...
          // Show unauthorized banner if both are unauthorized
      ...
      fetch('/detections_face')
        ...
        .then(data => {
          ...
          data.slice(-5).reverse().forEach(item => {
            ...
            details.innerHTML = `
              <div class=\"detection-name\">${item.type === 'known' ? item.name : 'Unknown'}</div>
    ...
    function updateCombinedDetections() {
      Promise.all([
        ...
        // Combine and sort all detections
        const allDetections = [
        ...
        allDetections.slice(-10).forEach(item => {
          ...
          let name = '';
          if (item.source === 'face') {
            name = item.type === 'known' ? item.name : 'Unknown Face';
          } else {
