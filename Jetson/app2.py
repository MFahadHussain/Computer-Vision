...
RTSP_URL = os.environ.get(
    "RTSP_URL",
    "rtsp://admin:afaqkhan-1@192.168.18.143:554/Streaming/channels/102",
)
...
# ---------------- Config Tweaks ----------------
FRAME_PROCESS_EVERY = 3          # process every 3rd frame (skip others)
BARRIER_COOLDOWN = 5             # seconds before barrier can open again for anyone
SAVE_UNKNOWN_ASYNC = True        # save unknown crops in background

last_barrier_open_time = 0.0
unknown_save_queue = queue.Queue()
...
threading.Thread(target=unknown_saver, daemon=True).start()
...
class Worker(threading.Thread):
    ...
    def _maybe_log_attendance(self, name: str):
        ...
        if now - last >= ATTENDANCE_DEDUP_WINDOW_SEC:
            ...
            # 🚦 Open barrier + log barrier event
            try:
                open_barrier()
                self._push_activity(
                    event="barrier",
                    details=f"Barrier opened for {name} at {ts_str}",
                )
                logger.info(f"Authorized face detected: {name} → Barrier opened")
            except Exception as e:
                logger.error(f"Failed to open barrier for {name}: {e}")
