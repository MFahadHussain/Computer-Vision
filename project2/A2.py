...
from PyQt5.QtGui import QPalette, QFont, QIcon, QImage, QPixmap, QPainter, QColor, QPen
...
class VideoInputHandler:
    ...
    def initialize(self) -> bool:
        """Initialize video capture with improved error handling."""
        with self.connection_lock:
            try:
                ...
                if self.fps <= 0 or self.fps > 60:  # Validate FPS
    ...
    def read_frame(self) -> Tuple[bool, Optional[np.ndarray]]:
        """Read a frame from the video source with improved reconnection logic."""
        ...
        try:
            ...
            if not ret:
                ...
                return self._handle_frame_read_failure()
            ...
            if self.frame_count % 30 == 0:
                ...
                if elapsed > 0:
                    ...
                    if abs(actual_fps - self.fps) > 5:  # Significant deviation
                        print(f"FPS deviation detected: Expected {self.fps:.2f}, Actual {actual_fps:.2f}")
...
class DetectionEngine:
    def __init__(self, model_path: str = "yolov8n.pt", detection_size: Tuple[int, int] = (640, 360), 
                 frame_skip: int = 2, use_threading: bool = True, confidence_threshold: float = 0.5):
    ...
    def initialize(self) -> bool:
        """Initialize YOLO model with improved error handling."""
        try:
    ...
    def _detection_worker(self):
        """Worker function for threaded detection with improved error handling."""
    ...
    def _detect_objects(self, frame: np.ndarray) -> List[Dict]:
        """Detect objects in frame using YOLO model with improved accuracy."""
        try:
            # Resize frame for detection while maintaining aspect ratio
            h, w = frame.shape[:2]
            ...
            results = self.model(resized_frame, verbose=False, conf=self.confidence_threshold)
    ...
    def set_confidence_threshold(self, threshold: float):
        ...
        print(f"Detection confidence threshold set to {self.confidence_threshold:.2f}")
...
# ================== IMPROVED POLYGON ZONE DETECTOR ==================
class PolygonZoneDetector:
    def __init__(self, config_file: str = "zone_config.json", cooldown: int = 3):
        ...
        self.zone_colors = []  # Colors for each zone
        self._generate_zone_colors()
    ...
    def _generate_zone_colors(self):
        ...
        self.zone_colors = colors
        
    def load_zones(self) -> bool:
        """Load zones from configuration file with validation."""
        try:
                ...
                with open(self.config_file, 'r') as f:
                    ...
                    for zone in zones:
                        if len(zone) >= 3:  # Need at least 3 points for a polygon
                            ...
                            print(f"Invalid zone with {len(zone)} points, skipping")
    ...
    def save_zones(self) -> bool:
        """Save zones to configuration file with backup."""
        try:
...
class AlertSystem:
    ...
    def _load_sounds(self):
        ...
        try:
            ...
            if self.siren_sound:
                self.siren_sound.set_volume(self.volume)
        except Exception as e:
            print(f"Error loading sounds: {e}")
    
    def _create_siren(self) -> Optional[pygame.mixer.Sound]:
        """Create a siren sound with improved quality."""
        try:
            sample_rate = 22050
            ...
            print(f"Error creating siren sound: {e}")
            return None
    
    def set_volume(self, volume: float):
        """Set the volume of the siren (0.0 to 1.0)."""
        self.volume = max(0.0, min(1.0, volume))
        if self.siren_sound:
            self.siren_sound.set_volume(self.volume)
        print(f"Siren volume set to {self.volume:.2f}")
    
    def start_siren(self):
        """Start playing the siren in a separate thread."""
        if not self.siren_playing and self.siren_sound:
    ...
    def trigger_zone_alert(self, frame: np.ndarray, zone_idx: int) -> Dict:
        ...
        # Save snapshot
        snapshot_filename = f"zone_alert_{timestamp.replace(':', '-')}.jpg"
        snapshot_path = os.path.join(self.alert_dir, snapshot_filename)
...
class Camera:
    def __init__(self, camera_id: str, source: str, config_file: str = None, 
                 ...
                 use_threading: bool = True, confidence_threshold: float = 0.5):
    ...
    def set_siren_volume(self, volume: float):
        """Update the volume of the siren."""
        self.alert_system.set_volume(volume)

# ================== IMPROVED CAMERA WIDGET ==================
class CameraWidget(QFrame):
    alert_signal = pyqtSignal(dict)
    snapshot_signal = pyqtSignal(str, str)  # camera_id, snapshot_path
    ...
    def __init__(self, camera_id, parent=None):
        ...
        self.snapshot_button.setMaximumSize(24, 24)
        ...
        indicator_layout.addWidget(self.siren_indicator)
        
        self.siren_label = QLabel("Siren: Off")
        self.siren_label.setStyleSheet("color: #888; font-size: 12px;")
        indicator_layout.addWidget(self.siren_label)
        ...
        # Statistics
        self.stats_label = QLabel("Frames: 0 | Detections: 0 | Alerts: 0")
    ...
    def take_snapshot(self):
        ...
        if self.current_frame is not None:
            ...
            snapshot_dir = os.path.join("snapshots", f"camera_{self.camera_id}")
            os.makedirs(snapshot_dir, exist_ok=True)
            
            snapshot_path = os.path.join(snapshot_dir, f"snapshot_{timestamp}.jpg")
            cv2.imwrite(snapshot_path, self.current_frame)
            
            print(f"Camera {self.camera_id}: Snapshot saved to {snapshot_path}")
            self.snapshot_signal.emit(self.camera_id, snapshot_path)
    ...
    def update_frame(self, frame_data):
        ...
        if self.siren_active:
            ...
            self.siren_label.setText("Siren: ON")
            self.siren_label.setStyleSheet("color: #ff5555; font-size: 12px;")
        else:
            self.siren_indicator.setStyleSheet("background-color: #333; border-radius: 8px;")
            self.siren_label.setText("Siren: Off")
            self.siren_label.setStyleSheet("color: #888; font-size: 12px;")
        ...
        self.stats_label.setText(f"Frames: {total_frames} | Detections: {total_detections} | Alerts: {total_alerts}")
        
        # Process frame for display
        display_frame = self.current_frame.copy()
        
        # Draw danger zones with distinct colors
        for i, zone in enumerate(self.zones):
            color = self.zone_detector.zone_colors[i % len(self.zone_detector.zone_colors)]
            
            # Draw filled polygon with transparency
            overlay = display_frame.copy()
            cv2.fillPoly(overlay, [np.array(zone, dtype=np.int32)], color)
            ...
            if len(zone) > 0:
                ...
                if M["m00"] != 0:
                    cX = int(M["m10"] / M["m00"])
                    cY = int(M["m01"] / M["m00"])
                    cv2.putText(display_frame, f"DANGER ZONE {i+1}", (cX - 60, cY), 
        ...
        if self.drawing and len(self.current_zone) > 0:
            ...
            cv2.putText(display_frame, "Double-click to finish zone", (10, 30), 
        ...
        for detection in self.detections:
            ...
            cv2.putText(display_frame, f"{confidence:.2f}", (bbox[0], bbox[1] - 10), 
            ...
            if zone_idx != -1:
                ...
                cv2.putText(display_frame, "ALERT!", (bbox[0], bbox[1] - 30), 
                ...
                if M["m00"] != 0:
                    cX = int(M["m10"] / M["m00"])
                    cY = int(M["m01"] / M["m00"])
                    cv2.line(display_frame, centroid, (cX, cY), (0, 0, 255), 2)
...
class AlertPanel(QFrame):
    view_snapshot_signal = pyqtSignal(str)
    ...
    def add_alert(self, alert_data):
        ...
        if snapshot_filename:
            alert_text += f" [{snapshot_filename}]"
        alert_text += "\n"
        ...
        # Add clickable link for snapshot
        if snapshot_filename:
            format.setForeground(QColor(100, 150, 255))
            format.setUnderlineStyle(QTextCharFormat.SingleUnderline)
            cursor.setCharFormat(format)
            cursor.insertText("View Snapshot\n")
    ...
    def export_alerts(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Export Alerts", "", "Text Files (*.txt)"
        )
        
        if file_path:
            try:
                with open(file_path, 'w') as f:
                    f.write(self.alert_list.toPlainText())
...
class ControlPanel(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self.detection_size_combo.currentIndexChanged.connect(self._on_settings_changed)
        ...
        self.frame_skip_spin.valueChanged.connect(self._on_settings_changed)
        ...
        """)
        alert_layout.addRow("Siren Volume:", self.volume_slider)
        ...
        self.cooldown_spin.valueChanged.connect(self._on_settings_changed)
        ...
        """)
        alert_layout.addRow("Alert Cooldown:", self.cooldown_spin)
        ...
        # Detection rate
        self.detection_rate_label = QLabel("0.0/sec")
        self.detection_rate_label.setStyleSheet("color: #55ff55;")
        status_layout.addRow("Detection Rate:", self.detection_rate_label)
    ...
    def _on_settings_changed(self):
        ...
        settings = {
            'detection_size': self.detection_size_combo.currentText(),
    ...
    def update_detection_rate(self, rate: float):
        """Update the detection rate display."""
        self.detection_rate_label.setText(f"{rate:.1f}/sec")
...
class Dashboard(QMainWindow):
    def __init__(self, camera_configs):
        ...
        # Set up timer for updating detection rate
        self.detection_rate_timer = QTimer(self)
        self.detection_rate_timer.timeout.connect(self.update_detection_rate)
        self.detection_rate_timer.start(5000)  # Update every 5 seconds
    ...
    def init_ui(self):
        ...
        for i, config in enumerate(self.camera_configs):
            ...
            camera_widget.snapshot_signal.connect(self.show_snapshot)
        ...
        self.control_panel.settings_changed_signal.connect(self.apply_settings)
    ...
    def start_system(self):
        ...
        for i, config in enumerate(self.camera_configs):
            camera = Camera(
                ...
                use_threading=True,
                confidence_threshold=self.control_panel.confidence_spin.value()
            )
    ...
    def stop_system(self):
        ...
        for camera_widget in self.camera_widgets:
            ...
            camera_widget.siren_label.setText("Siren: Off")
            camera_widget.siren_label.setStyleSheet("color: #888; font-size: 12px;")
    ...
    def apply_settings(self, settings):
        ...
        # Update detection size
        detection_size = self.get_detection_size_from_string(settings['detection_size'])
        ...
        for camera in self.cameras:
            camera.detection_engine.detection_size = detection_size
            camera.detection_engine.frame_skip = frame_skip
            camera.set_confidence_threshold(confidence)
            camera.set_siren_volume(volume)
            camera.zone_detector.cooldown = cooldown
        
        self.status_bar.showMessage("Settings applied to all cameras")
    
    def get_detection_size(self):
        size_str = self.control_panel.detection_size_combo.currentText()
        return self.get_detection_size_from_string(size_str)
    
    def get_detection_size_from_string(self, size_str):
        width, height = map(int, size_str.split('x'))
        return (width, height)
    
    def show_settings_dialog(self):
        """Show a settings dialog for advanced configuration."""
        dialog = QDialog(self)
        ...
        camera_group = QGroupBox("Camera Configurations")
        ...
        for i, config in enumerate(self.camera_configs):
            ...
            camera_form = QFormLayout(camera_frame)
            
            # Camera ID
            camera_id_label = QLabel(f"Camera {config['camera_id']}")
            camera_id_label.setStyleSheet("font-weight: bold;")
            camera_form.addRow(camera_id_label)
            
            # Source
            source_edit = QLineEdit(config['source'])
            source_edit.setObjectName(f"source_{i}")
            camera_form.addRow("Source:", source_edit)
            
            # Config file
            config_edit = QLineEdit(config.get('config_file', ''))
            config_edit.setObjectName(f"config_{i}")
            camera_form.addRow("Config File:", config_edit)
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            for i, config in enumerate(self.camera_configs):
                source_edit = dialog.findChild(QLineEdit, f"source_{i}")
                config_edit = dialog.findChild(QLineEdit, f"config_{i}")
                
                if source_edit:
                    config['source'] = source_edit.text()
                if config_edit:
                    config['config_file'] = config_edit.text()
            
            self.status_bar.showMessage("Camera configurations updated")
    ...
    def show_about(self):
        QMessageBox.about(
            ...
            "<li>Customizable danger zones</li>"
            "<li>Visual and audio alerts</li>"
            "<li>Snapshot and logging capabilities</li>"
    ...
    def show_instructions(self):
        ...
        <ol>
            <li>Click the "Start" button to begin monitoring</li>
            <li>Configure camera settings in the Control Panel</li>
        </ol>
        
        <h3>Drawing Danger Zones</h3>
        <ol>
            <li>Left-click on a camera view to start drawing a zone</li>
            <li>Continue left-clicking to add points to the zone</li>
            <li>Double-click to finish the zone</li>
            <li>Right-click to cancel drawing</li>
        </ol>
        
        <h3>Taking Snapshots</h3>
        <ol>
            <li>Click the camera icon in the camera header</li>
            <li>Snapshots are saved in the "snapshots" folder</li>
        </ol>
        
        <h3>Managing Alerts</h3>
        <ol>
            <li>Alerts appear in the Recent Alerts panel</li>
            <li>Click "Clear" to remove all alerts</li>
            <li>Click "Export" to save alerts to a file</li>
        </ol>
        
        <h3>Settings</h3>
        <p>Adjust detection parameters, alert settings, and camera configurations in the Control Panel and Tools menu.</p>
        """
    ...
    def show_snapshot(self, camera_id, snapshot_path):
        """Show a snapshot in a new window."""
        dialog = QDialog(self)
        dialog.setWindowTitle(f"Snapshot - Camera {camera_id}")
        ...
        open_button.clicked.connect(lambda: os.startfile(snapshot_path))
    ...
    def update_detection_rate(self):
        """Update the detection rate calculation."""
        if not self.running:
            return
        ...
        if elapsed > 0:
            # Calculate total detections across all cameras
            current_detection_count = sum(camera.total_detections for camera in self.cameras)
            detections_in_period = current_detection_count - self.last_detection_count
            
            # Calculate rate (detections per second)
            rate = detections_in_period / elapsed
            
            # Update display
            self.control_panel.update_detection_rate(rate)
