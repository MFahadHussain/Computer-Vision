...
class AlertSystem:
    def __init__(self, alert_dir: str = "alerts", camera_id: str = "", camera_location: str = ""):
    ...
    def trigger_zone_alert(self, frame: np.ndarray, zone_idx: int):
        ...
        # Log alert
        log_entry = f"[{timestamp}] Camera {self.camera_id} ({self.camera_location}): Person detected in zone {zone_idx}. Siren activated. Snapshot saved to {snapshot_path}\n"
...
class Camera:
    def __init__(self, camera_id: str, source: str, camera_location: str, config_file: str = None, 
        ...
        self.alert_system = AlertSystem(camera_id=camera_id, camera_location=camera_location)
    ...
    def initialize(self) -> bool:
        ...
        print(f"Initializing Camera {self.camera_id} ({self.camera_location})...")
...
class ZoneEditDialog(QDialog):
    """Dialog for editing zone coordinates manually."""
    def __init__(self, zone_points, parent=None):
        super().__init__(parent)
        self.zone_points = zone_points
        self.original_points = [point for point in zone_points]
        self.init_ui()
    ...
    def init_ui(self):
        ...
        # Instructions
        instructions = QLabel("Edit the coordinates of each point in the zone:")
        instructions.setStyleSheet("color: white; padding: 5px; font-weight: bold;")
        layout.addWidget(instructions)
        
        # Table for coordinates
        self.table = QTableWidget()
        self.table.setColumnCount(2)
        self.table.setHorizontalHeaderLabels(["X Coordinate", "Y Coordinate"])
        self.table.setRowCount(len(self.zone_points))
        
        # Populate table with current values
        for i, (x, y) in enumerate(self.zone_points):
        ...
        self.remove_point_btn.setStyleSheet("""
            QPushButton {
    ...
    def add_point(self):
        ...
        # Default to the last point's coordinates or center of the frame
        if row_count > 0:
            last_x = int(self.table.item(row_count - 1, 0).text())
            last_y = int(self.table.item(row_count - 1, 1).text())
        else:
            last_x, last_y = 320, 240  # Default center
            
        self.table.setItem(row_count, 0, QTableWidgetItem(str(last_x)))
        self.table.setItem(row_count, 1, QTableWidgetItem(str(last_y)))
    
    def remove_point(self):
        """Remove the selected point from the zone."""
        current_row = self.table.currentRow()
        if current_row >= 0 and self.table.rowCount() > 3:  # Keep at least 3 points
            self.table.removeRow(current_row)
        elif self.table.rowCount() <= 3:
            QMessageBox.warning(self, "Cannot Remove", "A zone must have at least 3 points.")
    
    def get_edited_points(self):
        """Get the edited zone points from the table."""
        points = []
        for row in range(self.table.rowCount()):
            ...
            if x_item and y_item:
                try:
                    ...
                    # If invalid, use the original value
                    points.append(self.original_points[row])
            else:
                # If missing, use the original value
                points.append(self.original_points[row])
...
class CameraWidget(QFrame):
    ...
    def __init__(self, camera_id, camera_location, parent=None):
        super().__init__(parent)
        self.camera_id = camera_id
        self.camera_location = camera_location
        self.camera = None
        ...
        # Location label
        self.location_label = QLabel(camera_location)
        self.location_label.setAlignment(Qt.AlignRight)
        self.location_label.setStyleSheet("color: #cccccc; font-size: 12px;")
        ...
        # Define Zone button
        self.define_zone_btn = QPushButton("Define Zone")
        self.define_zone_btn.setStyleSheet("""
            QPushButton {
                background-color: #2196F3;
                color: white;
                border: none;
        ...
        """)
        self.define_zone_btn.clicked.connect(self.toggle_define_mode)
        zone_controls_layout.addWidget(self.define_zone_btn)
        ...
        # Zone definition instructions
        self.zone_instructions = QLabel("Click 'Define Zone' to start creating a danger zone")
        self.zone_instructions.setAlignment(Qt.AlignCenter)
        self.zone_instructions.setStyleSheet("color: #ffcc00; font-size: 12px; padding: 5px;")
        self.zone_instructions.setVisible(False)
        layout.addWidget(self.zone_instructions)
    ...
    def toggle_define_mode(self):
        """Toggle zone definition mode."""
        self.define_mode = not self.define_mode
        
        if self.define_mode:
            # Enter define mode
            self.drawing = True
            self.current_zone = []
            self.define_zone_btn.setText("Cancel Zone")
            self.define_zone_btn.setStyleSheet("""
                QPushButton {
                    background-color: #F44336;
                    color: white;
            ...
            """)
            self.zone_instructions.setText("Click on the video to add points. Double-click to finish the zone.")
            self.zone_instructions.setVisible(True)
            print(f"Camera {self.camera_id}: Entered zone definition mode")
        else:
            # Exit define mode
            self.drawing = False
            self.current_zone = []
            self.define_zone_btn.setText("Define Zone")
            self.define_zone_btn.setStyleSheet("""
                QPushButton {
                    background-color: #2196F3;
                    color: white;
            ...
            """)
            self.zone_instructions.setVisible(False)
            print(f"Camera {self.camera_id}: Exited zone definition mode")
            ...
            if self.current_frame is not None:
                ...
                for i, zone in enumerate(self.zones):
                    if self._point_in_polygon((frame_x, frame_y), zone):
                        ...
                        # Add "Edit Coordinates" action
                        edit_action = menu.addAction("Edit Coordinates")
                        edit_action.triggered.connect(lambda checked, idx=i: self.edit_zone_coordinates(idx))
    ...
    def edit_zone_coordinates(self, zone_idx):
        """Open dialog to edit zone coordinates manually."""
        if 0 <= zone_idx < len(self.zones):
            zone = self.zones[zone_idx]
            dialog = ZoneEditDialog(zone, self)
            
            if dialog.exec_() == QDialog.Accepted:
                edited_zone = dialog.get_edited_points()
                self.zones[zone_idx] = edited_zone
                self.save_zones()
                print(f"Camera {self.camera_id}: Zone {zone_idx} coordinates updated")
                ...
                if event.button() == Qt.LeftButton:
                    if self.drawing:
                        ...
                        if len(self.current_zone) >= 3:
                            self.zone_instructions.setText(f"Zone has {len(self.current_zone)} points. Double-click to finish.")
                        else:
                            self.zone_instructions.setText(f"Zone has {len(self.current_zone)} points. Add at least {3-len(self.current_zone)} more points.")
                    else:
                    ...
                    if self.drawing and not self.define_mode:
                        # Cancel drawing only if not in define mode
                        self.drawing = False
                        self.current_zone = []
        ...
        if 0 <= pos.x() < self.video_label.width() and 0 <= pos.y() < self.video_label.height():
            ...
            if self.current_frame is not None:
                ...
                if len(self.current_zone) >= 3:
                    ...
                    # If in define mode, reset for next zone
                    if self.define_mode:
                        self.current_zone = []
                        self.zone_instructions.setText("Zone saved! Click to add points for next zone or click 'Cancel Zone' to finish.")
                    else:
                        self.drawing = False
                        self.current_zone = []
                else:
                    # Not enough points, show message
                    self.zone_instructions.setText(f"Need at least 3 points to create a zone. Currently have {len(self.current_zone)}.")
...
class AlertPanel(QFrame):
    def __init__(self, parent=None):
        ...
        self.alert_table.setHorizontalHeaderLabels(["Time", "Camera", "Location", "Zone"])
        ...
        self.alert_table.itemSelectionChanged.connect(self.show_alert_details)
        
        # Store alert data for details view
        self.alert_data = {}
    ...
    def add_alert(self, alert_data):
        ...
        camera_item = QTableWidgetItem(f"Camera {camera_id}")
        location_item = QTableWidgetItem(camera_location)
        zone_item = QTableWidgetItem(f"Zone {zone_idx}")
        
        # Set items
        self.alert_table.setItem(row, 0, time_item)
        self.alert_table.setItem(row, 1, camera_item)
        self.alert_table.setItem(row, 2, location_item)
        self.alert_table.setItem(row, 3, zone_item)
        
        # Store alert data
        self.alert_data[row] = alert_data
    ...
    def show_alert_details(self):
        ...
        selected_rows = self.alert_table.selectedItems()
        if not selected_rows:
            return
            
        row = selected_rows[0].row()
        if row in self.alert_data:
            alert = self.alert_data[row]
            
            details = f"Timestamp: {alert['timestamp']}\n"
            details += f"Camera ID: {alert['camera_id']}\n"
            details += f"Location: {alert['camera_location']}\n"
            details += f"Zone: {alert['zone_idx']}\n"
            details += f"Snapshot: {alert['snapshot_path']}\n"
    ...
    def clear_alerts(self):
        reply = QMessageBox.question(
            self, 'Clear Logs',
            'Are you sure you want to clear all alert logs?',
            QMessageBox.Yes | QMessageBox.No, QMessageBox.No
        )
...
class Dashboard(QMainWindow):
    ...
    def init_ui(self):
        ...
        for i, config in enumerate(self.camera_configs):
            ...
            camera_widget = CameraWidget(camera_id, camera_location)
    ...
    def show_about(self):
        QMessageBox.about(
            ...
            "<p>Features:</p>"
            "<ul>"
            "<li>Real-time video monitoring from multiple cameras</li>"
            "<li>Danger zone detection and alerts</li>"
            "<li>Detailed logs of security events</li>"
            "<li>Customizable zone configuration</li>"
            "</ul>"
            "<p>Version 1.0</p>"
        ...
        return [
            {
                ...
                'camera_location': 'Default Location',
