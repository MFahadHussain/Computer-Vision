...
class YOLOWorldROIDetector:
    ...
    def __init__(self, model_path: str = 'yolov8l-worldv2.pt', 
        ...
        """Initialize the YOLO-World detector with tracking option."""
        self.model = None
        ...
        if self.use_tracking:
            try:
                ...
                print(f"Tracker initialized: {self.tracker.tracker_type}")
            except Exception as e:
                logger.warning(f"Failed to initialize tracker: {str(e)}")
                logger.warning("Falling back to default tracking (no persistent IDs)")
                self.use_tracking = False
    
    def select_roi(self, frame: np.ndarray) -> ROIConfig:
    ...
    def process_video(self, input_path: str, output_path: str,
        ...
        try:
            ...
            tracker_type_str = "BoT-SORT (or fallback)" if self.use_tracking else "Default"
            print("\nStarting video processing...")
            print(f"Tracking method: {tracker_type_str}")
            ...
            if self.use_tracking and self.tracker:
                print(f"Tracking method used: {self.tracker.tracker_type}")
